...
//for some reason needs different init on win in event loop,
//but still called in the main code :(
...
void handleinput(input *to_populate,bool blocking,int sw,int sh){
  ...
  cleaninput(to_populate);
			...
			do {
						...
						to_populate->multitouch=true;
						if (to_populate->pxcenter!=-1){
							//continuous multitouch, not first event, we have a delta
							int deltax,deltay;
							//see if delta qualifies as drag, pythagore
							deltax=to_populate->xcenter-to_populate->lastxcenter; 
							deltay=to_populate->ycenter -to_populate->lastycenter; 
							int squarediag;
							squarediag=deltax*deltax+deltay*deltay;
							if(squarediag>DRAG_THRESHOLD*DRAG_THRESHOLD){
								LOGD("multidrag \n");
								to_populate->multidrag=true;
							}else{
								LOGD("below multidrag threshold,registering pinch  \n");
								to_populate->pinch= true;
								to_populate->pamount = event.mgesture.dDist;
...
void handleinput(input *to_populate,bool blocking,int sw,int sh){
			...
			do {
				...
				if(to_populate->multitouch){
					//we cancel other events
					to_populate->newpress=false;
					to_populate->newpress=true;
